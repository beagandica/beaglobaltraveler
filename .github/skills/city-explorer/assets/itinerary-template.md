# [City] itinerary: [Number] days

## Traveler profile

- **Vibe**: [foodie / culture / nightlife / chill / mix]
- **Budget**: [$ / $$ / $$$]
- **Dates**: [dates if known]

---

## Day 1: [Theme for the day]

### 🌅 Morning

**[Activity/Place name]**, [neighborhood]
- [Description and why it's worth visiting]
- 💰 Budget: [$ level]
- 🚇 How to get there: [transit directions]

### ☀️ Afternoon

**[Activity/Place name]**, [neighborhood]
- [Description]
- 💰 Budget: [$ level]
- 🚇 How to get there: [transit from morning spot]

**🍜 Lunch: [Restaurant name]**, [neighborhood]
- [What to order]
- 💰 Budget: [$ level]

### 🌙 Evening

**[Activity/Place name]**, [neighborhood]
- [Description]
- 💰 Budget: [$ level]
- 🚇 How to get there: [transit from afternoon spot]

**🍽️ Dinner: [Restaurant name]**, [neighborhood]
- [What to order]
- 💰 Budget: [$ level]

---

## Day 2: [Theme for the day]

[Repeat morning/afternoon/evening structure]

---

## Practical notes

- **Transit card**: [T-money, PASMO, Oyster, etc.]
- **Key app**: [Naver Map, Google Maps, Citymapper, etc.]
- **Local phrase of the trip**: [phrase] ([translation])

---

*Planned with ✈️ by Wanderly. Happy wandering!*
